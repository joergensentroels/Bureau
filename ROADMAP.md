...
- ✅ **Automated test suite (broad)** — `node test/run-all.mjs` runs everything: **157 pure assertions** (`decision` core; `units`: SSRF guard, action normalization, tolerant JSON, RAG terms + the pure `rankDeliverables` ranker, delegation matcher, goal/KR helpers, policy sanitizer, html→text, org normalization, DoD checklist rendering, deliverable-name validation; `net`: `apiCall`/`fetchUrl` parsing + SSRF, offline) and **57 server assertions** (`api` CRUD/validation across company, guardrails, notify, goals, policies, triggers, agents, deliverable lifecycle + versions, schedules, goal-cadence→schedule wiring, dashboard/runs/performance/audit — all inside a throwaway workspace; `workspaces` isolation), plus a 10-assertion live e2e (`--e2e`). Server-dependent suites auto-skip when no server is up. 18 pure helpers were `export`ed from `server.mjs` to make them testable. Fixed along the way: the deliverable-name regex had an unescaped `.` (accepted dotless names) — now a shared, escaped `validDeliverableName()`; and `retrieveRelevant` was split into a disk wrapper over the pure `rankDeliverables()`.
