...
- ✅ **Multi-workspace** — each workspace is a **fully separate company**: its own agents, goals, policies, guardrails, audit, schedules, triggers, deliverables (drafts), agent profiles, and Latch approvals (tagged `ws:<id>`, so each Inbox shows only its own). Your existing data *is* the "default" workspace — no rename, no migration. Implemented with `AsyncLocalStorage` so the ~65 persistence call sites resolve the right per-workspace files automatically; a per-workspace write lock; runs and the scheduler carry their workspace. Header switcher + create/rename/delete in the UI. Unknown workspace ids fall back to default. Covered by `test/workspaces.test.mjs` (11/11). **This surfaced and fixed a latent data-integrity bug**: `readOrg` spread a shared `EMPTY_ORG` template, so empty workspaces shared the same `agents`/`policies` arrays — now a fresh `emptyOrg()` per read.
- ✅ **Automated test suite (comprehensive, headless)** — `node test/run-all.mjs`: **262 assertions across 6 suites** (175 pure + 87 server), plus a 10-assertion live e2e (`--e2e`). Pure: `decision` core; `units` (SSRF guard, action normalization + passthroughs, tolerant JSON, RAG ranker, delegation matcher, goal/KR helpers, policy sanitizer, html→text, org normalization + legacy migration, DoD checklist, filename validation, work-product assembly); `net` (`apiCall`/`fetchUrl` parsing + SSRF + DNS-failure, offline). Server (all in throwaway workspaces): `api` CRUD/validation; `workspaces` isolation; `robustness` (404s, malformed/oversized input, 4 MB body cap → 413, unicode, **static-file traversal blocked**, run-lifecycle 404s, not-found sweep, numeric clamping, agent-field round-trips, the **concurrent-write mutex guarantee**). 21 helpers `export`ed from `server.mjs`. **Hardening fixes made along the way:** deliverable-name regex `.`→`\.` (shared `validDeliverableName()`); `retrieveRelevant` split over a pure `rankDeliverables()`; a `readBody` 4 MB cap (drains, doesn't reset the socket → clean 413); `ensureBudget` now self-sufficient on a bare `{}`. Coverage boundary: what's left is LLM/Latch-bound (delegation recursion, decompose, bio/HR generation, trigger firing, revision versioning) — the `--e2e` realm, not deterministically assertable.
...
_Parked but available:_ **paid-model activation / smart routing** — the biggest lever on output quality (local qwen3:8b is the ceiling); starts whenever a paid provider/API key is configured in Latch. Also still in the backlog: office-view revamp.
