...
- ✅ **Automated test suite (broad)** — `node test/run-all.mjs` runs everything: **127 pure assertions** (decision core + `units.test.mjs`: SSRF guard, action normalization, tolerant JSON, RAG terms, delegation matcher, goal/KR helpers, policy sanitizer, html→text, org normalization, DoD checklist rendering) and **57 server assertions** (`api.test.mjs` CRUD/validation across company, guardrails, notify, goals, policies, triggers, agents, deliverable lifecycle + versions, schedules, goal-cadence→schedule wiring, dashboard/runs/performance/audit — all inside a throwaway workspace; `workspaces.test.mjs` isolation), plus a 10-assertion live e2e (`--e2e`). Server-dependent suites auto-skip when no server is up. 14 pure helpers were `export`ed from `server.mjs` to make them unit-testable. The live e2e was re-confirmed 10/10 after the multi-workspace refactor.
