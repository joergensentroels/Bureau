...
- ✅ **Automated test suite (broad)** — `node test/run-all.mjs` runs everything: **116 pure assertions** (decision core + `units.test.mjs`: SSRF guard, action normalization, tolerant JSON, RAG terms, delegation matcher, goal/KR helpers, policy sanitizer, html→text) and **44 server assertions** (`api.test.mjs` CRUD/validation across company, guardrails, notify, goals, policies, triggers, agents, deliverable lifecycle — all inside a throwaway workspace; `workspaces.test.mjs` isolation). Server-dependent suites auto-skip when no server is up; the live model e2e runs with `--e2e`. 12 pure helpers were `export`ed from `server.mjs` to make them unit-testable.
