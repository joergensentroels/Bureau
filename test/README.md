...
Current totals: **<!--fig:pure-assertions-->1,867 pure assertions** across
**<!--fig:pure-suites-->23 pure suites**, plus **<!--fig:server-assertions-->311 server assertions**
across **<!--fig:server-suites-->6 server suites** — **<!--fig:assertions-->2,178 headless assertions
across <!--fig:suites-->29 suites** in all. The live `--e2e` adds 18 more and is not counted here,
...
| `ps1-encoding.test.mjs` | that no `.ps1` file carries a non-ASCII character anywhere PowerShell 5.1 will tokenise it. These files are UTF-8 with no BOM, so 5.1 decodes them as CP1252 and an em dash becomes three characters ending in `U+201D`, which it accepts as a closing quote — one em dash inside a live string ends it early and the rest of the file stops parsing. Comments stay exempt, because the prose in these files is full of them on purpose. It reads BYTES rather than running a parser: CI has pwsh 7, which decodes UTF-8 correctly and would pass exactly the file 5.1 chokes on. Self-checks against eight controls each run, and fails if it scans nothing. Enumerates via `git ls-files` rather than walking the tree: the walk wandered into `.claude/worktrees/`, another session's gitignored checkout of this repo, so it scanned 6 files locally and 3 in CI and its assertion count moved with local scratch state — green here, red on the runner. The count is now the eight controls plus one aggregate claim, so it does not move with the file list at all |
| `secret-tokens.test.mjs` | where the operator and read-only credentials come from, and that the strong source cannot degrade into a weak one. Precedence is secret file → environment variable → Latch's `auth.json`, and a *named* secret file that is missing or empty **fails the boot** rather than falling back — with a valid `auth.json` sitting right there, so a fallback would otherwise succeed silently and leave a working Bureau with no boundary. Also that a whitespace-only token is fatal (empty `TOKEN` locks every request out via `authRole`, so it would boot, listen, and refuse the operator's own UI in silence), and that a BOM or trailing CRLF is stripped. Each case runs in its own child process because `DATA_DIR` is fixed at module load; no token value is ever printed, only eight hex characters of SHA-256, because the fallback path reads the real operator token on a developer machine |
| `compose-boundary.test.mjs` | that `docker/compose.yaml` still keeps Bureau away from Latch's filesystem — no `LATCH_DATA`, no reference to Latch's repo path, no named volume mounted into both, every published port bound to `127.0.0.1`, and Latch's read-only rootfs plus the `LATCH_LOG` redirect that makes it survive boot. A comment asking people not to add that mount is not a control: it is the obvious fix the first time a container cannot find its token, it makes the error go away, and the stack then runs as one trust domain wearing two containers. Text-scoped rather than YAML-parsed (this repo has no dependencies), so it fails loudly if the file stops looking the way it expects instead of asserting nothing about a file it no longer understands. Its first version matched the *comment* warning against `LATCH_DATA` and read `LATCH_URL=…:8787` as a published port — both controls passed anyway, so the controls now run through the same decomment/sub-block chain the assertions do |
| `state-dir.test.mjs` | that `BUREAU_STATE_DIR` relocates every mutable path — and above all that its DEFAULT is the repo root, so an existing install resolves the database, org blobs, drafts, profiles and log exactly where it always did and has nothing to migrate. Getting that wrong brings a live Bureau up against an empty database while reporting itself healthy. Asserts the split from `server.mjs`'s own source (only `public/` may hang off `HERE`; anything written belongs on `STATE_DIR`), that `tools/backup.mjs` and `tools/hunt-log.mjs` import the shared resolver rather than restating it — a tool resolving it differently does not fail, it reads a *different database* and reports confidently about it — and then boots a real Bureau with the override set at a directory that does not exist, confirming it is created, that the DB lands inside it, and that the operator's live `data-bureau.db` is untouched |
| `harness.test.mjs` | the **continual harness** — durable, evidence-backed notes about WHERE a round should look, and above all what they may not do. This is the only register whose contents are written by a model and read back into a later prompt, so the load-bearing assertions are the negative ones: a hostile note reaches the rendered prompt (asserted, as a control) and still cannot widen `HUNT_ACTIONS`, lift the floor or reach a scoped-out file, because the enforcement is code and the note is data. Also the evidence asymmetry that justifies a second register at all — `normalizeLens` requires a CONFIRMED finding, so it can never learn "this lens misfires on a web app"; a note may rest on a round that looked and found nothing, but not on an `idle` one that examined nothing. Plus bounded rendering (the local 4,096-token context clips the FRONT of a prompt), eviction of notes that never helped, snapshot/rollback because a model wrote the state, and a wiring check derived from `investigateObjective`'s real output rather than from reading the call site |
| `provenance.test.mjs` | that untrusted content entering a run removes the STANDING grant for writes. Measured gap: the hard floor stops shell, api_call, email_draft, mcp_call and the GitHub communication actions, but an `autonomous` tier, a run-level autoApprove or a policy `allow` auto-approves `file_write`, `plan_add` and `github_file` — and `github_file` COMMITS to a repository while `read_issues` is safe-tier. So a scheduled autonomous run could read an attacker-filed issue and commit attacker-chosen content unattended. The suite asserts that gap EXISTS first, or "tainted → needs a human" is satisfied by an action that was never auto-approvable. Then: the three writes demote, a policy `allow` cannot re-open it, the floor stays the stronger statement in both directions, and — the permanent negative control — the taint changes the answer for **exactly** those three and nothing else, because a gate that refuses everything would satisfy every other assertion here. Arming is checked from source at `emitAct`, the one place all 42 dispatcher branches pass through |
